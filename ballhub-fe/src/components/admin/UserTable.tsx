...
import { Eye, Lock, Unlock, Key, ShieldAlert, ShieldCheck, Trash2 } from "lucide-react";
import { toast } from "sonner";
import api from "@/lib/axios";
import { ConfirmModal } from "@/components/common/ConfirmModal";
import { UserProfileModal } from "./UserProfileModal";

export const UserTable = ({ users = [], onRefresh }: { users?: any[], onRefresh: () => void }) => {
  ...
  const handleChangeRole = (user: any) => {
    ...
    setConfirmState({
      ...
      action: async () => {
        try {
          await api.put(`/users/admin/${user.userId}/change-role?role=${newRole}`);
  ...
  const handleResetPassword = (user: any) => {
    setConfirmState({
      ...
      action: async () => {
        try {
          const promise = api.post(`/users/admin/${user.userId}/reset-password`);
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          {users && users.length > 0 ? (
            users.map((user, index) => (
              <tr key={user.userId} className="hover:bg-slate-50/50 transition-colors group">
                <td className="py-4 text-slate-400 font-medium text-xs">{index + 1}</td>
                <td className="py-4">
                  <div className="flex items-center gap-3">
                    <img src={user.avatar || "/no-image.png"} className="w-9 h-9 rounded-full object-cover border border-slate-100" alt="avatar" />
                    <div>
                      <p className="font-bold text-xs text-slate-700">{user.fullName}</p>
                      <p className="text-[10px] text-slate-400">ID: #{user.userId}</p>
                    </div>
                  </div>
                </td>
                <td className="py-4">
                  <p className="text-slate-600 text-xs font-medium">{user.email}</p>
                ...
                <td className="py-4">
                  <button 
                    ...
                    className={`font-bold text-[10px] px-2.5 py-1 rounded-md flex items-center gap-1 w-fit transition-all ${
                      user.role === "ADMIN" ? "bg-purple-100 text-purple-600 hover:bg-purple-200" : "bg-blue-100 text-blue-600 hover:bg-blue-200"
                    }`}
                  >
                ...
                <td className="py-4">
                  <span className={`font-bold text-[10px] px-2.5 py-1 rounded-md ${
                    user.status ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"
                  }`}>
                ...
                <td className="py-4 text-right">
                  <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setSelectedUser(user); setShowProfileModal(true); }} className="p-2 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all" title="Xem hồ sơ">
                      <Eye size={16} />
                    </button>
                    <button onClick={() => handleResetPassword(user)} className="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all" title="Cấp lại mật khẩu">
                      <Key size={16} />
                    </button>
                    <button onClick={() => handleToggleStatus(user)} className={`p-2 rounded-lg transition-all ${user.status ? "text-slate-400 hover:text-rose-500 hover:bg-rose-50" : "text-rose-500 hover:text-emerald-500 hover:bg-emerald-50"}`} title={user.status ? "Khóa tài khoản" : "Mở khóa"}>
                      {user.status ? <Lock size={16} /> : <Unlock size={16} />}
                    </button>
                    <button onClick={() => handleDeleteUser(user)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Xóa vĩnh viễn">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
            ...
            <tr><td colSpan={6} className="py-8 text-center text-slate-400 text-sm">Không có người dùng</td></tr>
          )}
        </tbody>
      </table>
      ...
      {showProfileModal && selectedUser && (
        <UserProfileModal userId={selectedUser.userId} onClose={() => setShowProfileModal(false)} />
      )}
    </div>
  );
};