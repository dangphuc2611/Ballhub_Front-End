...
import { toast } from "sonner"; // ✅ Đảm bảo bạn đã cài đặt sonner
...
export function Header({ showSearch = true }: HeaderProps) {
  ...
  // ✅ STATE ĐIỀU KHIỂN MODAL ĐĂNG XUẤT
  ...
  const formatPrice = (v: number) => {
    try { return v.toLocaleString("vi-VN") + "đ"; } 
    catch { return v + "đ"; }
  };

  // Suggesstions logic... (giữ nguyên code cũ của bạn)
  useEffect(() => {
    ...
    if (suggestCacheRef.current.has(q)) {
      ...
      setOpenSuggest(true); setActiveIndex(-1); return;
    }
    const t = setTimeout(async () => {
      try {
        setLoadingSuggest(true);
        const params = new URLSearchParams();
        params.append("search", q); params.append("page", "0");
        params.append("size", "6"); params.append("sort", "new");
        ...
        const items: SuggestProduct[] = (json?.data?.content ?? []).map((p: any) => ({
            ...
            mainImage: p.mainImage, minPrice: Number(p.minPrice ?? 0),
        }));
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-8 ml-3">
            {menuItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-sm font-medium text-gray-700 hover:text-green-500 transition">
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          {showSearch && (
            <div ref={wrapRef} className="hidden lg:flex flex-1 mx-8 items-center gap-2 relative">
              ...
              {/* Suggestions Dropdown (giữ nguyên logic cũ của bạn) */}
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                ...
                {/* ✅ THAY ĐỔI: Gọi Modal thay vì logout ngay */}
                <Button
      ...
      {/* ✅ CUSTOM MODAL XÁC NHẬN ĐĂNG XUẤT (Gắn vào Header) */}
      {showLogoutModal && (
